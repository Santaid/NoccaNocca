...
public class Alpha_T : MonoBehaviour//MonoBehaviourやUnity系は必要か不明
{
	...
	public int from_x,from_z,to_x,to_z;
	...
	public void abAIways(int[,] board_state,int turn){
		...
		int findinglimit = 7;//何手先まで読むか　７くらいがベスト	
	...
	int[] maxlevelab(int alpha,int beta ,int limit,int Flimit,int[,] board_state,int turn,int fft,int ffy,int ftt,int fty) {
		List<int[]> Selectways =new List<int[]>();	
		bool gameend = DecideJudge(Opponent(turn),Change_state_to_top(board_state));//負けたかどうか判定
  		if(limit == 0||gameend){
  			return new int[5]{fft,ffy,ftt,fty,gameend==true?(-1000000):evaluate(board_state)};
 	 	}
		//可能な手を全て生成;
		Canmovepieces = MovePieces(Change_state_to_top(board_state),turn);	
		for(int d=0;d<Canmovepieces.Count;d++){
	...
	int[] minlevelab(int alpha,int beta,int limit,int Flimit,int[,] board_state,int turn,int fft,int ffy,int ftt,int fty) {
		List<int[]> Selectways =new List<int[]>();
		bool gameend =DecideJudge(Opponent(turn),Change_state_to_top(board_state));//勝ったかどうか判定
		if(limit == 0||gameend){
	 		return new int[5]{fft,ffy,ftt,fty,(gameend==true)?(1000000):evaluate(board_state)};
	  	}
		//可能な手を全て生成;
	...
	//駒の移動できる範囲を調べる
	List<int[]> canMove(int r,int l,int turn,int[,] board_state){
		List<int[]> res=new List<int[]>();
		for(int i=-1; i<=1; i++){
			for(int j=-1; j<=1; j++){
	...
	int[,] Change_state_to_top(int[,] board_state){
		int[,] newboard = new int[row+2,line+2];
		for(int a=1;a<row+1;a++){
			for(int b=1;b<line+1;b++){
				if((board_state[a,b]==1)||(3<=board_state[a,b]&&board_state[a,b]<=4)||(7<=board_state[a,b]&&board_state[a,b]<=10)){newboard[a,b] = Black;}
				else if((board_state[a,b]==2)||(5<=board_state[a,b]&&board_state[a,b]<=6)||(11<=board_state[a,b]&&board_state[a,b]<=14)){newboard[a,b] = White;}
			}
		}
		return newboard;
	}
